...
def _overlay_color_name(session):
...
def map_salami(
    ...
    selection : Atoms, optional
        Restrict segmentation to residues touched by this atom specification.
        Useful for limiting export to one chain or residue range within the
        supplied model.
    zone : float, optional
    ...
    movie_resolution : {"4k", "1080p"}, optional
        Longest-dimension target for the movie while preserving the starting
        scene aspect ratio. Default 1080p uses 1080 as the longest dimension,
        and 4k uses 3840.
    ...
    selected_residue_keys = None
    if selection is not None:
        model_atoms = structure.atoms.filter(structure.atoms.mask(selection))
        if len(model_atoms) == 0:
            raise UserError("The selection keyword did not match any atoms in the supplied model.")
        selected_residue_keys = {_residue_key(r) for r in model_atoms.unique_residues}
    ...
    try:
        ...
        if movie:
            ...
            def _delete_movie_overlays():
                try:
                    ...
                    pass
                try:
                    run(session, "scalebar off")
                except Exception:
                    pass

            def _set_movie_label(label_name, label_text):
                run(
                    session,
                    "2dlabels create "
                    f"{_quote_string(label_name)} "
                    f"text {_quote_string(label_text)} "
                    f"color {overlay_color} size 36 xpos 0.78 ypos 0.92",
                )

            def _set_movie_scalebar():
                bar_x = 0.08
                bar_y = 0.07
                run(session, f"scalebar 10 color {overlay_color} xpos 0.08 ypos 0.07 thickness 10")
                run(
                    session,
                    "2dlabels create "
                    f"{_quote_string(movie_scalebar_label_name)} "
                    f"text {_quote_string('10 Å')} "
                    f"color {overlay_color} size 28 xpos {bar_x + 0.02:.3f} ypos {bar_y + 0.035:.3f}",
                )
            ...
            try:
                run(
                    session,
                    f"movie record format png size {movie_width},{movie_height}",
                )
                _delete_movie_overlays()
                ...
                for index, (scene_name, label_text) in enumerate(movie_scenes, start=1):
                    _delete_movie_overlays()
                    run(session, f"scenes restore {_quote_string(scene_name)}")
                    run(session, "wait 1")
                ...
                run(
                    session,
                    f"movie encode {_quote_path(movie_path)} quality highest framerate 30 wait true",
                )
            finally:
                _delete_movie_overlays()
        ...
        if movie_path is not None:
            session.logger.info(f"Wrote movie to {movie_path}.")
    finally:
        try:
            try:
                run(session, f"surface unzone {map_model.atomspec}")
...
def register_command(logger):
    ...
    from chimerax.core.commands import BoolArg, CmdDesc, EnumOf, FloatArg, IntArg, SaveFolderNameArg, register
    ...
    desc = CmdDesc(
        keyword=[
            ...
            ("patches", IntArg),
            ("movie", BoolArg),
            ("movie_resolution", EnumOf(("4k", "1080p"))),
            ("segment_size", IntArg),
            ("image_height", IntArg),
